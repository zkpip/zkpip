...
The ZK ecosystem is fragmented.

Proof systems like Groth16, Plonk, Halo2, STARKs all exist, but there is no unified standard to validate, exchange, or bundle proofs.

Without such a layer, interoperability and adoption remain harder than they should be.
...
From its very first day, ZKPIP positions itself as the _Switzerland of Zero-Knowledge_.  
...
You don’t need to see all the internals to know that something is true.
...
If a single proof can compress millions of constraints, then multiple proofs can also be validated in batch.

And if that is possible, why not across different proof systems?
