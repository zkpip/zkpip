...
export const snarkjsGroth16: Adapter = {
  ...
  canHandle(bundle: any): boolean {
    const ps =
      getField<string>(bundle, ['proofSystem', 'meta.proofSystem', 'system', 'provingScheme']) ??
      '';
    const fw =
      getField<string>(bundle, ['framework', 'meta.framework', 'tool', 'library', 'prover.name']) ??
      '';
    ...
    const hasProof =
      bundle?.proof || bundle?.groth16Proof || bundle?.meta?.proof || bundle?.meta?.groth16Proof;
    const hasPublic =
      bundle?.publicSignals ||
      bundle?.publicInputs ||
      bundle?.meta?.publicSignals ||
      bundle?.meta?.publicInputs;
    const hasVkey =
      bundle?.verificationKey ||
      bundle?.vkey ||
      bundle?.vk ||
      bundle?.meta?.verificationKey ||
      bundle?.meta?.vkey ||
      bundle?.meta?.vk;
  ...
  async verify(bundle: any) {
    try {
      ...
      const vkeyCandidate = getField<any>(bundle, [
        'verificationKey',
        'vkey',
        'vk',
