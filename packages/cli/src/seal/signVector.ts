...
// Output includes envelopeId (naming standard), vectorUrn, signature, alg, kid, createdAt, optional meta.
...
export function signVector(args: SignVectorArgs): SealedVector {
  ...
  // 3) Ed25519 sign (message is the canonical JSON bytes)
  const sigBuf = nodeSign(null, Buffer.from(canon, 'utf8'), args.privateKeyPem);
  const signature = sigBuf.toString('base64');

  // 4) Envelope id (stable format; timestamp separated for traceability)
