...
function fileUri(absPath) { return pathToFileURL(absPath).toString(); }
...
function bumpFirstNumericStringDeep(obj) {
  ...
  const bump = (v) => {
    if (done) return v;
    if (typeof v === "string") {
      if (/^-?\d+$/.test(v)) { done = true; return (BigInt(v) + 1n).toString(); }
      if (/^0x[0-9a-fA-F]+$/.test(v)) { // hex: utolsó nibble flip (nem 'f'→'0')
        const hex = v.slice(2);
...
async function main() {
  ...
  // Kötelező inputok
  const vkeyPath = [path.join(dir, "verification_key.json"), path.join(dir, "vkey.json")].find(exists);
  const proofPath  = path.join(dir, "proof.json");
  const publicPath = path.join(dir, "public.json");

  if (!vkeyPath) { console.error("Missing verification_key.json (or vkey.json) in", dir); process.exit(4); }
  if (!exists(proofPath)) { console.error("Missing", proofPath); process.exit(4); }
  if (!exists(publicPath)) { console.error("Missing", publicPath); process.exit(4); }

  // Opcionális
  const cirPath  = exists(path.join(dir, "circuit.circom")) ? path.join(dir, "circuit.circom") : null;
  const wasmPath = firstMatch(dir, [".wasm"], ["circuit.wasm"]);
  const zkeyPath = firstMatch(dir, [".zkey"], ["circuit_final.zkey", "circuit_0000.zkey"]);
  if (!wasmPath) { console.error("Missing WASM (e.g., circuit.wasm) in", dir); process.exit(4); }
  if (!zkeyPath) { console.error("Missing ZKEY (e.g., circuit_final.zkey) in", dir); process.exit(4); }
  ...
  const proofHasWrapper = (typeof proofRaw === "object") && (("proof" in proofRaw) || ("publicSignals" in proofRaw));
  const proofObj = proofHasWrapper ? proofRaw.proof : proofRaw;
  const publicSignals = Array.isArray(proofRaw?.publicSignals) ? proofRaw.publicSignals
                      : Array.isArray(publicArray) ? publicArray
                      : [];

  if (!Array.isArray(publicSignals) || publicSignals.length === 0) {
    console.error("publicSignals missing/empty (from proof.json or public.json)"); process.exit(4);
  }
  ...
  if (invalidPublic.length > 0) {
    ...
    for (let i = 0; i < invalidPublic.length; i++) {
      ...
      if (typeof v === "string" && /^-?\d+$/.test(v)) { invalidPublic[i] = (BigInt(v) + 1n).toString(); break; }
      if (typeof v === "string" && /^0x[0-9a-fA-F]+$/.test(v)) {
        const hex = v.slice(2); const last = hex.slice(-1);
        invalidPublic[i] = "0x" + hex.slice(0, -1) + ((parseInt(last,16)^1).toString(16));
        break;
      }
      if (typeof v === "number" && Number.isFinite(v)) { invalidPublic[i] = v + 1; break; }
    }
  }
