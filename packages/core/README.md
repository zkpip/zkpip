...
- Validate proof-envelopes against canonical JSON Schemas
...
```bash
zkpip validate ./vectors/proof-envelope.json
```

Verify with an adapter:

```bash
zkpip verify --adapter snarkjs --envelope ./vectors/proof-envelope.json
```
