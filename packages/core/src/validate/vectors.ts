...
function normalizeSchemaRef(schemaRef?: string): CanonicalUrn | undefined {
  ...
  // régi URN → új URN
  switch (s) {
    case 'urn:zkpip:mvs.proof-bundle.schema.json':
      return CANONICAL_IDS.proofBundle;
    case 'urn:zkpip:mvs.cir.schema.json':
      return CANONICAL_IDS.cir;
    case 'urn:zkpip:mvs.verification.schema.json':
      return CANONICAL_IDS.verification;
    case 'urn:zkpip:mvs.issue.schema.json':
      return CANONICAL_IDS.issue;
    case 'urn:zkpip:mvs.ecosystem.schema.json':
  ...
  switch (tail) {
    ...
    case 'ecosystem.schema.json':
      return CANONICAL_IDS.ecosystem;
    case 'mvs.proof-bundle.schema.json':
      return CANONICAL_IDS.proofBundle;
    case 'mvs.cir.schema.json':
      return CANONICAL_IDS.cir;
    case 'mvs.verification.schema.json':
      return CANONICAL_IDS.verification;
    case 'mvs.issue.schema.json':
      return CANONICAL_IDS.issue;
    case 'mvs.ecosystem.schema.json':
      return CANONICAL_IDS.ecosystem;
  }
...
export async function validatePath(inputPath: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    const candidates = preferred
      ? [preferred, ...ALL_URNS.filter((u) => u !== preferred)]
      : ALL_URNS;
