...
// Common fixture paths used by the envelope "artifacts" branch
const FIX_ROOT = join(process.cwd(), 'fixtures', 'snarkjs-groth16', 'valid');
const VKEY_PATH   = join(FIX_ROOT, 'verification.json');
const PROOF_PATH  = join(FIX_ROOT, 'proof.json');
const PUBLIC_PATH = join(FIX_ROOT, 'public.json');
...
describe('validate.ts + pickSchemaId integration', () => {
  ...
  it('routes proof-envelope files by name to the proofEnvelope schema (valid example)', async () => {
    // Minimal valid ProofEnvelope according to the current schema:
    // required top-level + artifacts branch (vkey/proof/publicSignals as refs)
    const validEnvelope = {
      envelopeId: 'urn:uuid:00000000-0000-4000-8000-000000000000',
      schemaVersion: '0.1.0',
      proofSystem: 'groth16',
      curve: 'bn128',
      prover: { name: 'snarkjs', version: '0.7.x' },
      program: { language: 'circom', name: 'example' },
      artifacts: {
        vkey: { path: VKEY_PATH },
        proof: { path: PROOF_PATH },
        publicSignals: { path: PUBLIC_PATH }
        // optional: wasm/zkey can be added later if needed
      }
      // "result" is not required when artifacts has vkey+proof+publicSignals
    } as const;
  ...
  it('rejects invalid proof-envelope (missing publicSignals in artifacts)', async () => {
    // Same as above but leave out artifacts.publicSignals to fail "anyOf" branch
