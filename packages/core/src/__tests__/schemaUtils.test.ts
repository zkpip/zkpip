...
describe('schemaUtils.loadSchemaJson', () => {
  ...
  beforeAll(async () => {
    ...
    // canonical: tmp-schemas/mvs/proof-set.schema.json
    const canonicalPath = path.join(tmpRoot, 'mvs', 'proof-set.schema.json');
    await mkdir(path.dirname(canonicalPath), { recursive: true });
    await writeFile(canonicalPath, JSON.stringify({ ok: true }), 'utf8');

    // alias: tmp-schemas/mvs.proof-set.schema.json
    const aliasPath = path.join(tmpRoot, 'mvs.proof-set.schema.json');
  ...
  it('loads by URN → canonical path', async () => {
    const data = await loadSchemaJson('urn:zkpip:mvs.proof-set.schema.json', {
      schemasRoot: tmpRoot,
    });
