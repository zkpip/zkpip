...
const PUB_PATH   = join(FIX_ROOT, 'public.json');
...
describe('validate.ts + pickSchemaId integration (picker)', () => {
  ...
  it('routes proof-envelope files by name to the proofEnvelope schema (valid example)', async () => {
    ...
    // required top-level + artifacts branch (vkey/proof/publicSignals as file refs)
    const validEnvelope = {
  ...
  // (Deprecated manifest tests removed)
